...
## HTML5 setup and configuration

FlixelGDX supports web builds through the **TeaVM** backend (in the `flixelgdx-html5` module). TeaVM transpiles
Java bytecode into JavaScript and WebAssembly so your game can run in a browser.
...
match `FlixelHtml5Launcher`'s default (`flixelgdx-canvas`) or the value you pass to
...
The framework repo **does not require an Android SDK** to compile. By default, the **flixelgdx-android** module is **not included** in the build, so you can clone, build core/desktop/web, and contribute without installing the SDK. Testing on Android is done in a **separate test project** (see [Testing with a test project](#testing-with-a-test-project)); that test project has its own Android app and its own SDK. Install the SDK there when you need to run on device or emulator.
